...
const Button: React.FC<ButtonProps> = ({ children, color, onClick }) => {
  const ColorEffect = {
    ...
    "trans-blue":
      "bg-transparent text-[#1A8CD8] hover:bg-[#031018] border-twitter-light-gray border-[1px]",
  };
  return (
    <button
      onClick={onClick}
      className={`${ColorEffect[color]} my-2 flex w-full items-center justify-center gap-4 rounded-full  px-4 py-2.5 transition duration-300 `}
    >
